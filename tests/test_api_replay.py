...
# ---------- 2. replay tool_input_error run → 仍然 failed(replay 不"修复") ----------


def test_replay_tool_input_error_run_still_failed(client):
    """1/0 是用户输入错(TOOL_INPUT_ERROR,HTTP 400),replay 不会自动改好。"""
    post = client.post(
        "/agent/run",
        json={"message": "1/0", "tool": "calculator"},
    )
    original_run_id = post.json()["run_id"]
    assert post.json()["status"] == "failed"
    assert post.json()["error"]["code"] == "TOOL_INPUT_ERROR"
...
# ---------- 3. replay tool_not_found run → 仍然 failed + TOOL_NOT_FOUND ----------


def test_replay_tool_not_found_run_still_failed(client):
    post = client.post(
        "/agent/run",
    ...
    original_run_id = post.json()["run_id"]
    assert post.json()["status"] == "failed"
    assert post.json()["error"]["code"] == "TOOL_NOT_FOUND"
...
def test_replay_persists_new_run_without_modifying_original(client):
    ...
    assert new_get.json()["status"] == "completed"
